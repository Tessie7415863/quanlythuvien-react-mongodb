...
export default function Book() {
    ...
    const totalPages = listBooks.totalPages
    const FilterSidebar = () => (
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            >
                {isSidebarOpen ? <i class="fa-solid fa-x"></i> : <i class="fa-solid fa-bars"></i>}
            </button>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {listBooks?.result && listBooks.result.length > 0 ? (
                            listBooks.result.map((item) => (
                                <div key={item.id} className="bg-white rounded-lg shadow p-4 flex flex-col justify-between hover:shadow-lg transition">
                                    <div className="mb-4">
                                        <img
                                        ...
                                        <p className="text-gray-600 mb-1">Tác giả: {item.author.name}</p>
                                        <p className="text-gray-600 mb-1">Ngày xuất bản: {item.published_date}</p>
                                        <p className="text-gray-600 mb-1">Ngành: {item.major.name}</p>
                                    </div>
                                    <button
